...
const weatherCode2Type = (code) => {
  const [weatherType] = Object.entries(weatherTypes).find(([type, codeList]) =>
    codeList.includes(Number(code))
  ) || [""];
  return weatherType;
};
...
const WeatherIcon = ({ currentWeatherCode, moment }) => {
  ...
  useEffect(() => {
    setCurrentWeatherType(weatherMemo);
  }, [weatherMemo]);

  return (
    <IconContainer>{weatherIcons[moment][currentWeatherType]}</IconContainer>
  );
};
